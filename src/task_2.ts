...
const normalizeData = (unnormalizedData: IPost[]): IOutputObj => {
    const outputObj: IOutputObj = {
        byId: {},
        allIds: []
    }

    unnormalizedData.forEach((item: IPost) => {
        Object.defineProperty(outputObj.byId,  item.id, {
            value: item, enumerable: true, writable: true, configurable: true
        })
        outputObj.allIds.push(item.id)
    })
